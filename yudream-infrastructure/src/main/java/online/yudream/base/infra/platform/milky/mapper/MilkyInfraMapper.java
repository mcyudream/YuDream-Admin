...
public final class MilkyInfraMapper {
    ...
    public static MilkyConnectionDO toDataObj(MilkyConnection source, MilkyCredentialCipher cipher) {
        ...
        target.setName(source.getName()); target.setBaseUrl(source.getBaseUrl()); target.setEncryptedToken(cipher.encrypt(source.getToken(), source.getId())); target.setEnabled(source.isEnabled()); target.setCommandMenuImageMode(source.getCommandMenuImageMode()); target.setCommandMenuPublicBaseUrl(source.getCommandMenuPublicBaseUrl());
        return target;
    }
    public static MilkyConnection toDomain(MilkyConnectionDO source, MilkyCredentialCipher cipher) {
        ...
        return MilkyConnection.builder().id(source.getId()).version(source.getVersion()).createTime(source.getCreateTime()).updateTime(source.getUpdateTime())
                .name(source.getName()).baseUrl(source.getBaseUrl()).token(cipher.decrypt(source.getEncryptedToken(), source.getId())).enabled(source.isEnabled()).commandMenuImageMode(source.getCommandMenuImageMode()).commandMenuPublicBaseUrl(source.getCommandMenuPublicBaseUrl()).build();
    }
}
