...
public class PersistentSatoriOperationLogger implements SatoriOperationLogger {
    ...
    private void save(Long connectionId, String level, String category, String action, String detail) {
        ...
        try {
            SatoriOperationLog saved = repo.save(SatoriOperationLog.create(connectionId, level, category, action, detail));
            eventPublisher.publishEvent(new SatoriOperationLogPublished(saved));
