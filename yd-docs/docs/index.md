...
hero:
  ...
  tagline: 可直接当成品部署的管理后台，也可作为主框架二次开发；15 项平台能力按需加载，插件机制无限扩展
  actions:
    - theme: brand
      text: 快速开始 →
...
features:
  - icon: 🧩
    title: 开箱即用
    details: 拉取 backend / frontend / render-server 官方镜像即可运行，业务差异全部通过插件 JAR 扩展，不改主框架源码。
  ...
  - icon: ⚡
    title: 15 项平台能力双闸门
    details: SSE、WebSocket、MQ、Neo4j、AI/Agent、CMS 等能力按需动态加载，项目闸门 + 应用闸门双重管控，不用的能力零开销。
...
| 维度 | 内容 |
|---|---|
| 平台能力（15 项） | api-docs、cms、wiki、form、document-template、integration、sse、websocket、rabbitmq、neo4j、ai、agent、dataviz、milky、message-render，全部受项目闸门（`yudream.platform.capabilities.<code>.enabled`）与应用闸门双重管控，支持依赖级联禁用 |
...
| 你要做的 | `docker compose up -d` 拉起三个官方镜像，后台安装插件 JAR | 克隆源码，JDK 21 + Maven / Node + pnpm 开发 |
...
| 部署 | Docker Compose（backend / frontend / render-server + watchtower 自动更新） |
