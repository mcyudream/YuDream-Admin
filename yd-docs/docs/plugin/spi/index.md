...
配套前端契约是 `@yudream/plugin-sdk`，当前 npm 包版本 **1.7.0**（`yudream-frontend/packages/plugin-sdk/package.json`）。后端 `@PluginFrontend.sdkVersion` 填写宿主实际注入的 SDK 行为版本，目前为 `1.7.0`（运行时 `sdk.messaging` / `sdk.users` / `sdk.ai` / `sdk.files.thumbUrl` / `sdk.site`）。1.4.0 新增构建期导出 `./uno-config`，1.5.0 新增列表缩略图出口，1.6.0 产物样式降入 `@layer yudream-plugin`，1.7.0 新增公开站 `sdk.site`。
...
| 范围 | 变更 |
|---|---|
| 插件主题（2.25.0） | `@PluginTheme` + `PluginContext.registerTheme(...)` 注册 SITE/ADMIN 整套主题：互斥顶替与激活位持久化、`homePreset` 首页方案 + `pages[]` 整套页面按 `themeCode` 隔离导入；主题样式经 `/api/platform/plugins/{code}/assets/**` 下发 |
| 主题配置 schema（2.26.0） | `@PluginTheme(configSchema="...")` 声明 JAR 内 theme-config.json，宿主渲染独立配置页 `/platform/theme-center/config/{theme}`，值按主题持久化，模板以 `theme.config.*` 消费 |
| 主题块提供者（2.26.0） | `PluginThemeBlockProvider`（`code/name/supportedThemes/data(ctx)`）：插件经 `registerExtension` 向公开站模板贡献 `blocks.{code}` 数据，只暴露公开安全字段 |
| Vue 原生主题页（2.26.0） | `@PluginTheme(homeComponent, chromeComponent)` 让主题远程 Vue 组件接管公开站首页与页头页脚 |
| 第三方登录提供方（2.27.0） | `PluginExternalLoginProvider`：插件贡献 CAS/OIDC/OAuth2 等外部登录；宿主持有 state、回调、绑定与会话，插件只构造授权地址并换身份 |
