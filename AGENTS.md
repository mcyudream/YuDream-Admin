...
- 插件主题经 `@PluginTheme` 注册（一个插件一个主题，`scopes` 声明 SITE/ADMIN）：同 scope 同时只激活一个，启用新主题插件自动顶替旧插件并持久化激活位（`sys_setting` category `plugin-theme`），禁用/卸载即释放回落内置主题。主题样式资产放插件 frontend 目录并经 `/api/platform/plugins/{code}/assets/**` 下发；CSS 约定 SITE 只写 `.site-page`/`.site-chrome` 与 `--yb-site-*` 变量、ADMIN 只写 `:root`/`.dark` 宿主变量，两 scope 不得互相污染。启动页皮肤也属主题：宿主 `loading.html` 只保留默认动画，禁止把主题 GIF/字体放进宿主 `public/`；SITE 主题用自己的 style.css 覆盖 `[data-app-loading]`，theme-runtime 在 router 就绪前按 URL 公开前缀打开 SITE media。细则见 `docs/plugin-system/specification.md` 9.1。
