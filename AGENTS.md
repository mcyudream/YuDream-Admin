...
- SITE 主题可用 `homePreset` 声明 JAR 内首页方案 JSON，激活时宿主导入为 `plugin:{pluginCode}` 方案并应用；**每个主题的首页设计彼此独立、绝不混杂**——导入前先还原到「主题接管前」基准（存为 `plugin-backup:{pluginCode}` 备份方案，不进方案列表），再整套应用本主题设计，上一个主题声明的 settings 键（如上套 `homeHtml`）不会残留；方案只覆盖声明的字段与 settings 键，未声明的（如 `navigationJson`）保留站点自有值。主题停用/顶替/卸载时首页仍被其占用则整体还原备份并删除备份。方案可再带 `pages[]` 整套页面——按 slug 归属创建/覆盖发布（标记 `sourcePluginCode`，管理员与他主题占用的 slug 跳过不覆盖），声明清单之外的该插件旧页面转草稿，主题停用/顶替/卸载时全部下线转草稿、再启用恢复发布。主题页面与 `settings.homeHtml` 一律用 `data-yb-*` 模板指令注入系统数据，禁止声明 `navigationJson`。后台「平台 → 主题中心」（独立一级菜单，已合并 CMS 页面/首页定制/导航/媒体库/首页方案）统一管理与一键切换；既有部署的种子菜单需手工清理旧「内容站点」节点。公开页换肤正途是插件适配主题：插件自带 `--xx-*` 桥接变量回退消费 `--yb-site-*`，主题按插件专属类名写样式仅作兼容层。
