...
- Developer tooling is host-side only and lives in `platform/devtools` (backend four layers) plus the `plugin-devtools` frontend store/drawer; Agent execution tracing lives in `platform/agent`. Plugins need zero adaptation, and none of this touches `yudream-plugin-spi` contracts. See `docs/plugin-system/dev-mode.md`.
