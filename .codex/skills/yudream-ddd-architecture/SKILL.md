...
- SITE theme plugins may declare Vue native pages: `@PluginTheme(homeComponent="theme/Home", chromeComponent="theme/Chrome")` names remote-module route keys (`[a-z0-9][a-z0-9/-]*/[A-Za-z][A-Za-z0-9]*`). `homeComponent` takes over `/site`; `chromeComponent` takes over public-site header/footer (host injects navigation data and always prepends a Home `/site` item, the theme paints chrome so page switches never restyle the bar). Remaining pages use `@PluginRoute(publicAccess=true, siteNav=true)`. Themes must `softdepend` business plugins, never `depend`. Without `chromeComponent` the host SiteChrome remains and may still consume the chrome CSS-variable contract. Splash chrome is also plugin-owned: host `loading.html` stays the generic animation and must not ship theme GIF/fonts in `public/`; a SITE theme restyles `[data-app-loading]` from its own assets, and `theme-runtime` opens SITE stylesheet media from the URL public-path prefix before Vue Router is ready. Details: `docs/plugin-system/specification.md` 9.1 and `references/knowledge.json` keys `plugin-theme-vue-pages` / `site-chrome-var-contract` / `plugin-theme-splash`.
