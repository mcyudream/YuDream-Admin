...
- EnvironmentPostProcessor property lookups must read raw values from property sources. `environment.getProperty("spring.data.mongodb.uri")` throws `PlaceholderResolutionException` when application.yml holds an unresolved `${MONGO_URI}` placeholder — exactly the fresh-install case the installer boots in.
