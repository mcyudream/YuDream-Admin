...
- CMS visual editing uses `@myissue/vue-website-page-builder` on the frontend. Store page builder output as the complete saved HTML, including the outer `#pagebuilder` wrapper: normal pages use `htmlContent`, and the homepage uses `HomePageLayout.settings.homeHtml` unless a dedicated backend field is later added. Keep Markdown/HTML editing and legacy homepage sections as fallback modes instead of replacing the publish pipeline with a custom block format.
