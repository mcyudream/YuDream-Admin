...
- Plugin dev mode is gated by `yudream.platform.plugin.dev-mode.enabled` (default `false`) and must log a loud startup warning when on. Dev projects load from `target/classes` + `target/plugin-dev/lib/*.jar` and win over same-code JARs; `PluginDevModeWatcher` debounces source/compile-output/frontend-dist changes into compile, disable→unload→load→enable, and frontend-remount events. Compile failures are published as events and must never reload stale output or crash the host.
