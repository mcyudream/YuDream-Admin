...
- `list.vue` 使用 `<script setup lang="ts">`，导入 `type { TableColumn } from '@yudream/components'`。
...
- `DetailForm` 使用 Arco Design Vue `FormInstance` / `FieldRule`，通过 `useTemplateRef<FormInstance>('formRef')` 获取表单引用，并 `defineExpose({ submit() { ... } })` 给外部调用。
