  ...
  <section class="qq-workspace">
    ...
    <aside class="conversation-rail">
      ...
      <template v-if="activeView !== 'notifications'">
        ...
        <div class="peer-scroll">
          <button v-for="peer in visiblePeers" :key="`${peer.scene}-${peer.id}`" class="peer-row" :class="{ selected: selected?.id === peer.id && selected?.scene === peer.scene }" @click="openPeer(peer)">
            <FaBadge :value="Number(peer.unread) || 0" badge-class="start-[100%] -translate-x-[50%]"><FaAvatar :src="peer.avatar || ''" :fallback="peer.name.slice(0, 1)" class="size-[42px]" /></FaBadge>
      ...
      <template v-else>
        ...
        <div class="peer-scroll notification-list"><article v-for="item in notifications" :key="item.request_id || item.notification_id" class="notification-row"><FaAvatar :src="item.avatar || ''" :fallback="String(displayName(item)).slice(0, 1)" class="size-[38px]" /><div><b>{{ displayName(item) }}</b><p>{{ item.comment || item.message || '新的申请' }}</p><span><FaButton size="sm" @click="actNotification(item, true)">同意</FaButton><FaButton size="sm" variant="ghost" @click="actNotification(item, false)">拒绝</FaButton></span></div></article><a-empty v-if="!notifications.length" description="暂无通知" /></div>
      </template>
    </aside>

    <main class="chat-stage">
      <header class="chat-header">
        <template v-if="selected"><FaAvatar :src="selected.avatar || ''" :fallback="selected.name.slice(0, 1)" class="size-[38px]" /><div><h2>{{ selected.name }}</h2><p>{{ selected.scene === 'group' ? `群聊 ${selected.id}${selected.memberCount ? ` · ${selected.memberCount} 人` : ''}` : `私聊 ${selected.id}` }}</p></div></template>
      ...
      <div ref="messageScroll" class="message-scroll">
        ...
        <article v-for="message in messages" :key="message.message_seq" class="message-row" :class="{ self: message.sender_id === selected?.id }">
          <FaAvatar :src="message.sender_avatar || ''" :fallback="String(message.sender_nickname || message.sender_id || '?').slice(0, 1)" class="size-[36px]" />
