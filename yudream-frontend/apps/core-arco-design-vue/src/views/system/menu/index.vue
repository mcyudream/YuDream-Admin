...
import type { MenuManageItem, MenuNodeType, MenuPayload, MenuStatus } from '@/api/modules/system-menu'
...
const selectedParent = computed(() => selectedMenu.value ? findMenu(rows.value, selectedMenu.value.parentCode) : undefined)
...
const buttonColumns = computed<TableColumn<MenuManageItem>[]>(() => [
  ...
  { accessorKey: 'permission', header: '权限字符', minWidth: 240 },
  { id: 'status', header: '菜单状态', width: 120, align: 'center' },
...
function openCreate(parent?: MenuManageItem) {
  ...
  Object.assign(form, {
    ...
    module: parent?.module || selectedParent.value?.module || selectedMenu.value?.module || '系统管理',
    icon: '',
    path: '',
    component: '',
    link: '',
...
function openCreateFromSelected() {
  openCreate(canCreateChild.value ? selectedMenu.value : undefined)
}
...
function normalizePayload(): MenuPayload {
  const payload: MenuPayload = {
    ...
    path: form.type === 'MENU' || form.type === 'LAYOUT' || form.type === 'LINK' ? form.path || undefined : undefined,
    component: form.type === 'MENU' ? form.component || undefined : form.type === 'LAYOUT' || form.type === 'LINK' ? 'Layout' : undefined,
    link: form.type === 'LINK' ? form.link || undefined : undefined,
    permission: form.type !== 'CATEGORY' && form.type !== 'LAYOUT' ? form.permission || form.code : undefined,
    sort: form.sort ?? 0,
  }
  if (editing.value) {
    delete payload.code
...
<template>
  <div class="menu-page">
    ...
    <aside class="menu-sidebar">
      ...
      <FaInput v-model="searchKeyword" clearable placeholder="请输入菜单名称" class="w-full" @keydown.enter="loadTree" @clear="resetSearch" />
      ...
      <section class="menu-section">
        <div class="menu-section__header">
          ...
          <div class="menu-actions">
            ...
            <FaButton v-auth="'system:menu:delete'" variant="destructive" size="sm" :disabled="!selectedMenu || selectedMenu.status === 'DISABLED'" @click="confirmDisable()">
              <FaIcon name="i-ri:delete-bin-line" />
              删除
            </FaButton>
          </div>
        ...
        <div v-if="selectedMenu" class="detail-grid">
          ...
          <div class="detail-cell detail-cell--label">权限字符</div>
          <div class="detail-cell">{{ displayValue(selectedMenu.permission) }}</div>
          <div class="detail-cell detail-cell--label">上级菜单</div>
          <div class="detail-cell">{{ selectedParent?.name || '根目录' }}</div>
          ...
          <div class="detail-cell detail-cell--label">显示状态</div>
          <div class="detail-cell">
            <FaTag :variant="statusVariant(selectedMenu.status)">
              {{ statusText(selectedMenu.status) }}
            </FaTag>
        ...
        >
          ...
          <template #cell-operation="{ row }">
            <div class="flex-center gap-2">
              ...
              <FaButton v-auth="'system:menu:delete'" variant="ghost" size="sm" title="删除" :disabled="row.original.status === 'DISABLED'" @click="confirmDisable(row.original)">
                <FaIcon name="i-ri:delete-bin-line" />
              </FaButton>
            </div>
          </template>
        ...
        <a-grid :cols="2" :col-gap="16">
          <a-grid-item>
            <a-form-item label="菜单编码" required>
              <FaInput v-model="form.code" :disabled="!!editing" placeholder="system:menu" class="w-full" />
            </a-form-item>
          </a-grid-item>
          <a-grid-item>
          ...
          <a-grid-item>
            <a-form-item label="模块">
              <FaInput v-model="form.module" placeholder="系统管理" class="w-full" />
            </a-form-item>
          </a-grid-item>
          <a-grid-item>
          ...
          <a-grid-item v-if="form.type === 'MENU' || form.type === 'LAYOUT' || form.type === 'LINK'">
          ...
          <a-grid-item v-if="form.type === 'LINK'">
            <a-form-item label="外链地址">
              <FaInput v-model="form.link" placeholder="https://fantastic-admin.hurui.me" class="w-full" />
            </a-form-item>
          </a-grid-item>
...
.menu-tree-node__main span:last-child {
  overflow: hidden;
