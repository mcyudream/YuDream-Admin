...
- **监听管线**（`PluginDevModeWatcher`，启动时按生效开关决定是否起线程）：
  ...
  3. 前端 `dist` 变化 → 发布前端重载事件，经 SSE 桥到调试浮窗，触发当前插件运行时页面重挂载远程模块（重挂载会重置页面状态，不是状态保持的 HMR）。调试浮窗「插件」详情与「设置」页另有「前端重载」按钮，对应 `POST /plugins/{code}/frontend-reload`，不走 Java disable/unload，只发布同一 FRONTEND_RELOAD 事件；浏览器按实时 `assetRevision` 加唯一 import URL 打破 ESM 缓存后再挂载。
...
- **插件**：主从结构——先插件清单（名称、状态、开发模式徽标与来源），点入某插件后分组展示其运行时贡献，按开发关注度排序：HTTP 端点、QQ 指令、前端模块与路由 → 权限、菜单 → AI 工具、声明式 Agent → 平台能力、消息交互、首页卡片、服务导出。端点测试器与指令模拟器在插件详情内，开发模式且已启用的插件可一键「前端重载」（只重挂载远程模块）与「Java 重载」（disable→unload→load→enable）。未启用（LOADED/ERROR）的插件详情提供「启用」按钮，且资产区展示未启用横幅——Java 重载不会自动启用从未启用过的插件，贡献全 0 属预期。清单工具栏可切换「依赖图」视图：按插件展示依赖（depend）/可选依赖（softdepend）/被依赖/被可选依赖四向关系，指向未安装插件的依赖标红；每张卡片提供「禁用预览」，弹窗列出禁用该插件的级联影响——启用中的传递硬依赖方（按建议禁用顺序排列，运行时拒绝禁用存在启用中硬依赖方的插件）、启用中的直接软依赖方（禁用后其可选集成降级）、已加载的直接依赖方（存在时卸载/重载将被拒绝）。
...
- **设置**：开发项目管理（登记/批量登记子目录/启用/移除/前端重载/Java 重载，含来源标记与路径/编译/描述符状态位）+ 新建插件骨架（表单填父目录与编码，宿主生成独立 Maven 模块并默认登记为开发模式项目）+ 面板偏好（悬浮按钮位置、浮窗位置与尺寸一键重置）。
...
| `POST /plugins/{code}/reload` | manage | 手动 Java 重载（开发模式插件：disable→unload→load→enable） |
| `POST /plugins/{code}/frontend-reload` | manage | 手动前端热重载（仅开发模式且已启用；发布 FRONTEND_RELOAD，不回收 Java 运行时） |
...
- **前端改动不生效**：确认插件前端在 `vite build --watch`，且最近动态出现 FRONTEND_RELOAD；也可在「插件」详情或「设置」页点「前端重载」。重挂载会按实时 assetRevision 加唯一 import URL 打破浏览器 ESM 缓存，并重置页面状态，不是状态保持的 HMR。
