...
public final class AgentTemplateNodeHandler implements AgentWorkflowNodeHandler {
    ...
    public AgentWorkflowNodeResult execute(AgentWorkflowNode node, AgentWorkflowContext context) {
        return values.result(node, values.render(values.text(node, "template"), context));
    }
}
