...
public class ExternalLoginWebAssembler {
    private ExternalLoginWebAssembler() {
    }
    ...
    public static ExternalLoginAuthorizationRes toRes(ExternalLoginAuthorizationDTO d) {
        return ExternalLoginAuthorizationRes.builder().authorizationUrl(d.getAuthorizationUrl()).state(d.getState()).build();
    }

    public static ExternalLoginCallbackRes toRes(ExternalLoginCallbackDTO d) {
        return ExternalLoginCallbackRes.builder().outcome(d.getOutcome().name()).session(d.getSession()).bindingToken(d.getBindingToken())
                .providerCode(d.getProviderCode()).type(d.getType()).nickname(d.getNickname()).avatarUrl(d.getAvatarUrl()).build();
    }

    public static ExternalAccountRes toRes(ExternalAccountDTO d) {
        return ExternalAccountRes.builder().id(d.getId()).providerCode(d.getProviderCode()).platformType(d.getPlatformType()).socialUid(d.getSocialUid())
                .nickname(d.getNickname()).avatarUrl(d.getAvatarUrl()).createTime(d.getCreateTime() == null ? null : d.getCreateTime().toString()).build();
    }
}
