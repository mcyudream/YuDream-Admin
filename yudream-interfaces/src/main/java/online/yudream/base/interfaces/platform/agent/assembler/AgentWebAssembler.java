...
public final class AgentWebAssembler {
    ...
    public static AgentRunCmd toRunCmd(Long id, AgentRunRequest request, SecurityPrincipal principal) { AgentRunCmd cmd = new AgentRunCmd(); cmd.setApplicationId(id); cmd.setInput(request.getInput()); cmd.setProviderCode(request.getProviderCode()); cmd.setModelCode(request.getModelCode()); cmd.setImageDataUrl(request.getImageDataUrl()); cmd.setAttachments(request.getAttachments() == null ? java.util.List.of() : request.getAttachments().stream().map(item -> new AgentAttachmentCmd(item.getName(), item.getContentType(), item.getSize(), item.getDataUrl())).toList()); if (principal != null) { cmd.setPermissionCodes(principal.permissions() == null ? java.util.List.of() : java.util.List.copyOf(principal.permissions())); cmd.setPermissionContextExplicit(true); } return cmd; }
