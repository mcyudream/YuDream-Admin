...
public final class AgentWebAssembler {
    ...
    public static AgentRunCmd toRunCmd(Long id, AgentRunRequest request) { AgentRunCmd cmd = new AgentRunCmd(); cmd.setApplicationId(id); cmd.setInput(request.getInput()); cmd.setProviderCode(request.getProviderCode()); cmd.setModelCode(request.getModelCode()); cmd.setImageDataUrl(request.getImageDataUrl()); return cmd; }
