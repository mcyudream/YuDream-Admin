...
import online.yudream.base.application.platform.wiki.service.WikiIngestProgressAppService;
...
public class WikiIngestProgressStreamSupport {

    private static final long TIMEOUT_MILLIS = 600_000L;

    private final WikiIngestProgressAppService service;

    public SseEmitter subscribe(Long spaceId) {
        SseEmitter emitter = new SseEmitter(TIMEOUT_MILLIS);
        AutoCloseable subscription = service.subscribe(spaceId,
        ...
        try {
            emitter.send(SseEmitter.event().name("wiki.ingest").data(java.util.Map.of(
                    "event", "wiki.ingest", "action", "subscribed", "module", "wiki")));
        }
    ...
    private void send(SseEmitter emitter, Object data) {
        try {
            synchronized (emitter) {
                emitter.send(SseEmitter.event().name("wiki.ingest").data(data));
            }
        }
        catch (IOException error) {
            emitter.completeWithError(error);
