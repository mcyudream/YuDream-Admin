...
tar -tf "$PLUGIN_SDK_TGZ" | grep -q '^package/uno\.config\.js$' || fail "plugin-sdk tarball must contain uno.config.js"
tar -tf "$PLUGIN_SDK_TGZ" | grep -q '^package/uno\.config\.d\.ts$' || fail "plugin-sdk tarball must contain uno.config.d.ts"
tar -tf "$PLUGIN_SDK_TGZ" | grep -q '^package/src/uno-config\.ts$' || fail "plugin-sdk tarball must contain src/uno-config.ts"
